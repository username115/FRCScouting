...
public class SaveLoadClickListener implements OnClickListener {
	...
	public SaveLoadClickListener(final FileOperation operation,
			final FileSelector fileSelector, final Context context) {
	...
	public void onClick(final View view) {
		...
		if (mOperation == FileOperation.SELECTDIR || checkFileName(text)) {
			final String filePath = mFileSelector.getCurrentLocation()
					.getAbsolutePath() + File.separator + text;
			...
			if (messageText != 0) {
				// Access denied.
				final Toast t = Toast.makeText(mContext, messageText,
						Toast.LENGTH_SHORT);
