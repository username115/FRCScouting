...
class VersionUpdater:
    ...
    def getVersionString(self, default):
        ...
        instr = input("Please enter a new Version String [" + default + "]:")
        if len(instr) < 1:
            self.ver = default
            return default
        self.ver = instr
    ...
    def getVersionCode(self, default):
        ...
        instr = input("Please enter a new Version Code (increment this if version string is updated) [" + default + "]:")
        if len(instr) < 1:
            self.verNum = default
            return default
        self.verNum = instr
    ...
    def getVersionDate(self, default):
        if self.versionDate is None:
            if self.noPrompt:
                ...
                instr = input("Please enter a date for the version update, or enter 'today' for today [" + default + "]:")
                if len(instr) < 1:
                    self.versionDate = default
                else:
                    self.versionDate = instr
    ...
    def getDevEmail(self, default):
        ...
        instr = input("Please enter the lead developer's email address [" + default + "]:")
        if len(instr) < 1:
            self.devEmail = default
            return default
        self.devEmail = instr
    ...
    def getdbVersion(self, default):
        ...
        instr = input("Please enter the database version number [" + default + "]:")
        if len(instr) < 1:
            self.dbVer = default
            return default
        self.dbVer = instr
    ...
    def UpdateManifest(self, manifestFileName, verbose=False):
        try:
            ...
            if verbose:
                print("Error updating manifest: " + manifestFileName + ": " + str(e))
    ...
    def UpdateValsFile(self, valFileName, verbose=False):
        try:
            ...
            if verbose:
                print("Error updating values: " + valFileName + ": " + str(e))
    ...
    def UpdateServerHeader(self, serverFileName, verbose=False):
        try:
            ...
            if verbose:
                print("Error updating server header: " + serverFileName + ": " + str(e))
    ...
    def UpdateDBVersion(self, dbHelperFileName, verbose=False):
        try:
            ...
            if verbose:
                print("Error updating db helper: " + dbHelperFileName + ": " + str(e))
...
def init_args():
    ...
    parser.add_argument('-lsf', '--legacyServerfile', dest='legacyServerFilename', required=False,
        help='Server header filename')
    ...
    parser.set_defaults(versionString=None,
                         ...
                         legacyServerFilename='FRC_Scouting_Server_Legacy/scouting-header.php',
...
if __name__ == "__main__":
    ...
    Version.UpdateServerHeader(args.legacyServerFilename, verbose=True)
